...
export default function ViewAvailabilityModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 p-4 animate-in fade-in duration-300">
      <div
        className={`${bgPrimary} border-2 ${border} rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-2xl animate-in zoom-in duration-500 transition-colors`}
      >
        {/* Header */}
        <div className={`relative bg-chart-2 p-6 border-b-2 ${border}`}>
          ...
          <div className="relative flex gap-3 mt-4">
            <div className="flex items-center gap-2 bg-white/20 backdrop-blur-sm px-4 py-2 rounded-xl border border-white/30">
              ...
              <span className="text-white text-sm font-bold">{sortedDates.length} Days</span>
            </div>
            <div className="flex items-center gap-2 bg-white/20 backdrop-blur-sm px-4 py-2 rounded-xl border border-white/30">
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-200px)] custom-scrollbar">
          {sortedDates.length === 0 ? (
            <div className="text-center py-16">
              <div
                className={`inline-flex p-6 ${emptyIconBg} rounded-full border-2 ${emptyIconBorder} mb-4`}
              >
