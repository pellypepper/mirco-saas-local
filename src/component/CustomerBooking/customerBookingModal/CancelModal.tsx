...
const CancelModal = ({
  ...
  const { reason, setReason, setSuccessOpen, successOpen, loading, handleCancel } =
    ...
    >
      ...
      >
        ...
        <div className="p-6 space-y-6">
          {/* BOOKING SUMMARY */}
          <div className={`rounded-2xl p-5 space-y-4 border-2 ${surfaceSoft} ${border}`}>
            <div className="flex items-center gap-3">
              <div className="p-2 bg-chart-2 rounded-lg">
                <Calendar className="w-5 h-5 text-white" />
              </div>
              <div>
                <p className={`text-xs font-bold uppercase ${textMuted}`}>Date</p>
          ...
          <div className={`flex gap-3 pt-4 border-t-2 ${border}`}>
            ...
            >
              ...
              <span className="relative">{loading ? 'Cancelling...' : 'Yes, Cancel'}</span>
            </button>
          </div>
        </div>
      </div>
