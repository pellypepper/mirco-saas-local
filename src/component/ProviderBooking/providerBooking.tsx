  ...
  return (
    <div className={`min-h-screen ${bgPrimary} p-4 md:p-8 transition-colors duration-300`}>
      ...
      <div className="fixed inset-0 pointer-events-none">
        <div
          className={`absolute top-0 right-1/4 w-96 h-96 bg-chart-2 ${ambientOpacity} rounded-full blur-3xl`}
        ></div>
        <div
          className={`absolute bottom-1/4 left-1/4 w-80 h-80 bg-chart-2 ${ambientOpacity} rounded-full blur-3xl`}
        ></div>
      </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-8">
          {[
            ...
            <div key={index} className="group relative">
              {/* Glow effect */}
              <div
                className={`absolute inset-0 bg-gradient-to-br ${stat.bgGradient} to-transparent opacity-0 group-hover:opacity-20 blur-xl rounded-2xl transition-opacity duration-500`}
              ></div>
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  {stat.badge ? (
                    <div
                      className={`${stat.badgeBg} ${stat.badgeText} px-3 py-1 rounded-full border-2`}
                      style={{ borderColor: `${stat.color}40` }}
                    >
                ...
                <p
                  className={`text-xs md:text-sm font-bold ${textSecondary} mb-1 uppercase tracking-wider`}
                >
                  {stat.label}
                </p>
                <p className={`text-3xl md:text-4xl font-black ${textPrimary}`}>{stat.value}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className={`${bgSecondary} border-2 ${border} rounded-2xl shadow-xl p-4 md:p-6 mb-8`}>
          <div className="flex flex-col md:flex-row gap-3 md:gap-4">
            <div className="relative flex-1 group">
              ...
              <Search
                className={`absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 ${textSecondary} group-focus-within:text-chart-2 transition-colors z-10`}
              />
              <Input
          ...
          {search && (
            <div
              className={`mt-4 flex items-center gap-2 text-sm ${searchResultBg} border-2 ${searchResultBorder} px-4 py-3 rounded-xl`}
            >
              <Search size={14} className="text-chart-2" />
              <span className={searchResultText}>
                Found <strong className="text-chart-2 font-black">{filteredBookings.length}</strong>{' '}
                booking{filteredBookings.length !== 1 ? 's' : ''} matching "
                <strong className={textPrimary}>{search}</strong>"
              </span>
            </div>
          )}
        </div>
