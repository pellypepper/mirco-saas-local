...
import { Mail, Lock, X, CheckCircle, AlertCircle, Shield, Key } from 'lucide-react';
        ...
        <div className="bg-chart-2 p-6 flex justify-between items-center">
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p className="text-white/90 text-sm">Update your email address</p>
            </div>
          </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className={`flex gap-3 pt-4 border-t-2 ${border}`}>
            ...
            >
              ...
              <span className="relative">{loading ? 'Updating...' : 'Update Email'}</span>
            </button>
          </div>
...
export const ChangePasswordModal = ({ onClose }: { onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in">
      <div
        className={`${border} ${surface} rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in`}
      >
        {/* Header */}
        <div className="bg-chart-2 p-6 flex justify-between items-center">
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p className="text-white/90 text-sm">Update your password</p>
            </div>
          </div>
        ...
        <div className="p-6 space-y-5">
          ...
          <div
            className={`flex gap-3 pt-4 border-t-2 ${isDarkMode ? 'border-zinc-700' : 'border-zinc-200'}`}
          >
            <button
              onClick={onClose}
              disabled={loading}
            ...
            <button
              ...
              disabled={
                loading || !oldPassword || !newPassword || !confirmPassword || !passwordsMatch
              }
...
const PasswordInput = ({
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="relative">
        <span className={`absolute left-4 top-1/2 -translate-y-1/2 ${textMuted}`}>{icon}</span>
        <input
          id={inputId}
...
const InfoNote = ({ text, isDarkMode }: { text: string; isDarkMode: boolean }) => (
  <div
