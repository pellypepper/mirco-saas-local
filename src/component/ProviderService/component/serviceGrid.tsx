...
const ServiceGrid = ({
  ...
  const deleteBtnHover = isDarkMode
    ? 'hover:bg-chart-1/30 hover:border-chart-1'
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {services.length === 0 ? (
        <div
          className={`col-span-full ${emptyCardBg} border-2 ${cardBorder} rounded-3xl shadow-2xl p-16 text-center`}
        >
          <div
            className={`inline-flex p-8 ${emptyIconBg} rounded-3xl border-2 ${emptyIconBorder} mb-6`}
          >
              ...
              <div className="relative p-6 space-y-5">
                ...
                <div>
                  <div className="flex items-start justify-between mb-3">
                    <h3
                      className={`text-xl font-black ${textPrimary} leading-tight group-hover:text-chart-2 transition-colors flex-1`}
                    >
                      {service.title}
                    </h3>
                    <div
                ...
                {/* Price & Duration Card */}
                <div
                  className={`relative ${infoBg} border-2 ${infoBorder} rounded-2xl p-5 space-y-4`}
                >
                  {/* Hover glow */}
...
<span className='text-2xl'>  {getCurrencySymbol(service.currency)}</span>
                    ...
                    <div className={`flex items-center gap-3 pb-4 border-b ${dividerBorder}`}>
                      <div
                        className={`p-2 ${isDarkMode ? 'bg-chart-1/20' : 'bg-red-50'} rounded-lg`}
                      >
                    ...
                    <div className="pt-3">
                      <p
                        className={`${perSessionText} text-xs font-black uppercase tracking-wider`}
                      >
                        Per Session
                      </p>
                    </div>
