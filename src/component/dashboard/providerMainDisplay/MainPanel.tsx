  ...
  return (
    <div className="grid lg:grid-cols-3 gap-6 mb-10">
      <CardGlass isDarkMode={isDarkMode} className="lg:col-span-2">
        <h2
          className={`text-xl font-extrabold flex items-center gap-3 mb-5 ${isDarkMode ? 'text-white' : 'text-black'}`}
        >
        ...
        <div className="space-y-3">
          {upcoming.length === 0 ? (
            ...
            upcoming.map((b: any) => (
              <div key={b.id} className="p-4 bg-chart-3/10 rounded-xl border border-chart-3/20">
                <p className={`font-semibold ${isDarkMode ? 'text-white' : 'text-black'}`}>
                  {b.customer?.full_name}
                </p>
                <p className={`text-sm ${isDarkMode ? 'text-chart-2' : 'text-chart-2'}`}>
                  {b.services?.title}
                </p>
              </div>
            ))
      ...
      <CardGlass isDarkMode={isDarkMode}>
        <h2
          className={`text-xl font-extrabold mb-5 flex items-center gap-2 ${isDarkMode ? 'text-white' : 'text-black'}`}
        >
