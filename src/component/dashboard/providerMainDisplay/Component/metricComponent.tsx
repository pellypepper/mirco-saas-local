...
export function CardGradient({ children, isDarkMode }: { children: any; isDarkMode: boolean }) {
  return (
    <div
      className={`cursor-pointer rounded-2xl p-4 md:p-5 ${isDarkMode ? 'bg-zinc-800 border-chart-2/20 hover:bg-chart-2/20' : 'bg-white border-gray-300 hover:bg-gray-100'} shadow-lg border hover:scale-[1.03] transition-all duration-150 backdrop-blur-sm`}
    >
      {children}
    </div>
  );
...
export function TrendPercent({ percent, isDarkMode }: { percent: number; isDarkMode: boolean }) {
  ...
  return (
    <div
      className={`text-xs font-bold flex flex-col items-center ${percent >= 0 ? 'text-chart-1' : 'text-red-600'} gap-1`}
    >
      {percent >= 0 ? Up : Down}
      {percent > 0 ? '+' : ''}
      {percent}%
