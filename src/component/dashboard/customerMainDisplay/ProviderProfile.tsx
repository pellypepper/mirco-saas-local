...
export function ProviderProfile({
  ...
  const groupedSlots = availability.reduce(
    (acc, slot) => {
  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? 'bg-zinc-900' : 'bg-white'}`}>
      ...
      <div
        className={`${isDarkMode ? 'bg-zinc-900/80 border-b-2 border-zinc-800' : 'bg-white/80 border-b-2 border-gray-300'} backdrop-blur-xl sticky top-0 z-40 shadow-2xl`}
      >
        <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-5">
          ...
          >
            <div
              className={`p-2 rounded-xl border-2 ${isDarkMode ? 'bg-zinc-800 border-zinc-700 group-hover:border-white ' : 'bg-white border-gray-300 group-hover:border-chart-2  '}  transition-colors duration-300`}
            >
              <ArrowLeft
                size={20}
      ...
      <div className="max-w-8xl mx-auto py-8 px-4">
        <div
          className={` rounded-3xl shadow-2xl overflow-hidden mb-8 border-2 ${isDarkMode ? 'border-zinc-700 bg-zinc-800' : 'border-gray-300 bg-white'}`}
        >
          {/*  Header Design */}
