              ...
              >
                ...
                <div className="flex items-center gap-2 mb-4">
                  <div className="flex items-center gap-1 px-4 py-2 rounded-xl">
                    {[...Array(5)].map((_, i) => (
                      <Star key={i} size={14} fill="currentColor" className={textPrimary} />
                    ))}
                  </div>
                  <span className="text-chart-2 font-bold text-lg">{provider.rating || '5.0'}</span>
                  <span className={`text-sm ${textMuted}`}>(127 reviews)</span>
                </div>

                {/* Name */}
                <h1 className={`text-2xl md:text-5xl font-black mb-4 leading-tight ${textPrimary}`}>
