...
export function LandingProviderSearch({
  providers,
  loading,
  onSelectProvider,
}: {
  ...
  const serviceTypes = [
    'All Services',
    ...Array.from(new Set(providers.map((p) => p.service_type))),
  ];
  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? 'bg-zinc-900' : 'bg-white'}`}>
      {/* Hero Section */}
      <div
        className={`relative ${isDarkMode ? 'bg-zinc-900 text-white' : 'bg-white text-black'} py-20 px-4 md:px-8 mb-8 overflow-hidden`}
      >
        <div className="relative max-w-7xl mx-auto">
          <div className="text-center mb-14">
            {/* Main Heading  */}
            <h1 className="text-5xl md:text-7xl font-black mb-6 leading-tight">
              <span className={`block ${isDarkMode ? 'text-white' : 'text-black'}`}>
                Find Your Perfect
              </span>
            ...
            <p
              className={`text-md md:text-xl ${isDarkMode ? 'text-gray-400' : 'text-zinc-700'} font-medium max-w-2xl mx-auto`}
            >
          ...
          <div className="flex flex-col md:flex-row gap-4 items-stretch max-w-5xl mx-auto">
            ...
            <div className="relative flex-grow group">
              ...
              <div className="relative">
                <Search
                  className="absolute left-5 top-1/2 -translate-y-1/2 text-zinc-500 group-focus-within:text-chart-2 transition-colors duration-300 z-10"
                  size={22}
                />
                <input
                  type="text"
