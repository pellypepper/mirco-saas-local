...
const ProviderGrid = ({
  ...
  return (
    <div className={`rounded-3xl mx-auto px-6 py-12 ${isDarkMode ? 'bg-zinc-900' : 'bg-white'}`}>
      ...
      <div className="mb-10">
        <h2
          className={`text-xl md:text-3xl font-black ${isDarkMode ? 'text-white' : 'text-black'} mb-2`}
        >
          Available Providers
        </h2>
        <div className="flex items-center gap-3">
          ...
          <p
            className={`text-zinc-400 font-medium text-sm ${isDarkMode ? 'text-zinc-400' : 'text-zinc-600'}`}
          >
            <span className="text-chart-2 font-bold text-base ">{filteredProviders.length}</span>{' '}
            service providers ready to help
          </p>
        </div>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="relative flex flex-col items-start px-6 pt-16 pb-6">
                {/* Name */}
                <h3
                  className={`text-xl font-bold ${isDarkMode ? 'text-white' : 'text-black'} mb-3 text-center group-hover:text-chart-2 transition-colors duration-300`}
                >
                  {p.full_name}
                </h3>
                ...
                <div className="flex items-center justify-center gap-2 mb-4">
                  <div
                    className={`border px-4 py-2 rounded-xl ${isDarkMode ? 'border-chart-2/20' : 'border-chart-2/20'}`}
                  >
                    <div className="flex items-center gap-2">
                      <Award size={14} className={isDarkMode ? 'text-chart-2' : 'text-chart-2'} />
                      <span
                        className={`text-sm font-bold ${isDarkMode ? 'text-chart-2' : 'text-chart-2'}`}
                      >
                        {p.service_type || 'Not Service available'}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                {/* Location */}
                <div
                  className={`flex items-center justify-center gap-2 text-sm mb-6 ${isDarkMode ? 'text-white/80' : 'text-zinc-400'}`}
                >
                  <MapPin size={14} className={isDarkMode ? 'text-white' : 'text-chart-2'} />
                  <span className={`${isDarkMode ? 'text-white/80' : 'text-zinc-700'}`}>
                    {p.country || 'Not location available'}
                  </span>
                </div>

                {/* Bio */}
                <div
                  className={` rounded-xl p-4 mb-6 border ${isDarkMode ? 'border-chart-2/20 bg-chart-3/20' : 'border-chart-2/20 bg-chart-3/20'} `}
                >
                  <p
                    className={`${isDarkMode ? 'text-white/80' : 'text-black'} text-sm line-clamp-3 leading-relaxed`}
                  >
