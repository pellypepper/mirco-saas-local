...
export default function AdminDashboard() {
  ...
  const [revenueData, setRevenueData] = useState<
    { month: string; revenue: number; bookings: number; customers?: number }[]
  >([]);
  ...
  const [recentCustomers, setRecentCustomers] = useState<
    { id: string; name: string; email: string; joined: string; bookings: number; spent: number }[]
  >([]);
  const [categoryData, setCategoryData] = useState<
    { category: string; bookings: number; revenue: number }[]
  >([]);
    ...
    return (
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 hover:shadow-lg transition-all duration-300 group">
        <div className="flex items-start justify-between mb-4">
          ...
          {showTrend && (
            <div
              className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-semibold ${isPositive ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'}`}
            >
              {isPositive ? (
          ...
          <div className="bg-white rounded-xl p-4 shadow-sm border border-slate-200 flex items-center gap-3 hover:shadow-md transition-shadow">
            ...
            <div className="flex-1">
              ...
              <p className="text-xl font-bold text-slate-900">
                {realtimeStats.activeBookings ?? 0}
              </p>
            </div>
          </div>
            ...
            <div className="space-y-3">
              {topProviders.length > 0 ? (
                topProviders.map((p, i) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 flex items-center justify-center rounded-full bg-gradient-to-br from-indigo-500 to-violet-500 text-white font-bold">
                        {i + 1}
                      </div>
                      <div>
